$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'hospital/nurse/list',
        datatype: "json",
        colModel: [			
...
var vm = new Vue({
	...
	methods: {
		...
		update: function (event) {
            ...
            vm.getInfo(id);
            vm.getDept();
            vm.getTitles(vm.nurse.deptId);
            vm.getTeams(vm.nurse.deptId);
            $.ajaxSettings.async = true;//开启异步

		},
		saveOrUpdate: function (event) {
            var content= ue.getContent();
            vm.nurse.introduce = encode64(strUnicode2Ansi(content));//加密，解决html在转json时乱码
            var url = vm.nurse.id == null ? "hospital/nurse/save" : "hospital/nurse/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.nurse),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "hospital/nurse/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "hospital/nurse/info/"+id, function(r){
                vm.nurse = r.nurse;
                if(vm.nurse.status==1){
                    $('#mySwitch input').bootstrapSwitch('state',true);
                }else{
                    $('#mySwitch input').bootstrapSwitch('state',false);
                }
                var htmlStr = strAnsi2Unicode(decode64(vm.nurse.introduce));//解密富文本内容
                ue.ready(function() {
        ...
        getDept: function(){
            ...
            $.get(baseURL + "sys/dept/list", function(r){
                ...
                var node = ztree.getNodeByParam("deptId", vm.nurse.deptId);
                if(node != null){
                    ztree.selectNode(node);
                    vm.nurse.deptName = node.name;
                }
            })
        },
        ...
        deptTree: function(){
            layer.open({
                ...
                btn1: function (index) {
                    ...
                    //选择上级科室
                    vm.nurse.deptId = node[0].deptId;
                    vm.nurse.deptName = node[0].name;
                    vm.getTitles(vm.nurse.deptId);
                    vm.getTeams(vm.nurse.deptId);
                    //TODO：设置角色多选框
